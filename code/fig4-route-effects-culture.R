# This file: - Plots the effects of route on all metrics, when monitoring via viral culture 
...
# Subset to dose of 10^4 pfu, culture data, from relevant tissues, & fixed cofactor set
df <- subset(df, dose_total == 4 & tissue_idx %in% c(1, 4, 6) & assay_idx == 4 &
...
fig.traj.culture.exposed <- 
  ...
  theme(text = element_text(size = 11),
        ...
        panel.grid.minor = element_line(size = 0.08, linetype = 'solid',
                                        colour = "light grey"),
        strip.background = element_rect(fill = "white", color = "black"),
        strip.text = element_blank()); fig.traj.culture.exposed
...
fig.traj.indiv.culture <- 
  ...
  scale_y_continuous(breaks = seq(0, 12, 2)) +
  coord_cartesian(xlim = c(0, 12), ylim = c(0, 6), clip = "on") +
  ...
  theme(text = element_text(size = 11),
        ...
        panel.grid.minor = element_line(size = 0.08, linetype = 'solid',
                                        colour = "light grey"),
        strip.background = element_rect(fill = "white", color = "black"),
        strip.text = element_text(size = 11)); fig.traj.indiv.culture


## Legend --------------------------------------------------------------

fig.for.legend <- ggplot(df) +
...
fig <- ((((fig.percent + fig.first + 
              ...
              fig.last  + 
              fig.duration +
              plot_layout(ncol = 2)) / fig.traj.culture.exposed) + plot_layout(heights = c(4, 1))) |  
           fig.traj.indiv.culture)  + plot_layout(widths = c(2.5, 1)) +
...
ggsave('./outputs/figures/fig4-route-effects.pdf',
       plot = fig,
       width = 7.5, 
       height = 10,
       dpi = 600)
