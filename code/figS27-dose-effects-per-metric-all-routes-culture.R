# This file: - Plots as lines the relationships between dose & main metrics, for culture and all routes
...
fig.dose.titer <-ggplot(df.dose.quantiles) + 
  ...
  scale_x_continuous(breaks = seq(1, 7, 1),
                     ...
                     expand = c(0, 0)) +
  coord_cartesian(clip = "off") +
  labs(x = "Exposure dose (pfu)", y = "Viral titer (log10 pfu)") +
...
ggsave("./outputs/figures/figS27-dose-effects-per-metric-all-routes-culture.png",
       plot = fig.dose.ed,
       width = 12, 
       height = 8,
       dpi = 600)
